<template>
    ...
    <div id="profile" class="absolute z-50 w-full items-center h-16  justify-between ">
        <div class="bg-gradient-to-r from-[#1d1f20] text-sm to-[#141516] tracking-wider pl-32 w-full h-full flex items-center space-x-4 px-4 drop-shadow-lg">


            <button id="menu-icon" >
                <svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" class="w-12 h-12">
                    <title>new</title>
            ...
            <button id="inventory-button" class=" h-full w-36 hover:bg-black/20">
                <span class="text-white/80 font-serif cursor-pointer">INVENTORY</span>
            </button>
            <button class=" h-full w-36 hover:bg-black/20"> 
                <span class="text-white/80 font-serif cursor-pointer">BRANCH</span>
            </button>
            <button id="equipment-button" class="h-full w-36 hover:bg-black/20">
                <span class="text-white/80 font-serif cursor-pointer">EQUIPMENT</span>
            </button>

            <div class="hidden w-52 h-10 flex space-x-10 font-serif text-xl text-white items-center">
                <div class="flex justify-center items-center w-auto h-full space-x-4">
                    <img class="w-8 h-8 shard" src="/src/assets/GUI/Coins/chess-pawn-svgrepo-com.svg">
                    <span>{{ chessShardsValue}}</span>
                </div>
                <div class="flex  justify-center items-center w-auto h-full space-x-4">
                    <img class="w-8 h-8 shard"  src="/src/assets/GUI/Coins/crown-svgrepo-com.svg">
                    <span>{{ battleShardsValue}}</span>
                </div>
...
export default {
    ...
    data(){
        return{
            chessShardsValue: null,
            battleShardsValue: null
        }
    },
    computed: {
    characters() {
            return this.$store.getters.playerProfile;
            }
        },
    methods: {
        chessShards(){
            this.chessShardsValue = this.$store.getters.playerProfile.chessShards;
            this.battleShardsValue = this.$store.getters.playerProfile.battleShards;
        },
    },
    mounted(){
        this.chessShards()
